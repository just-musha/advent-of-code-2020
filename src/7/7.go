...
func parseBagsInfo(bags map[string]NextBags, str string) {
	...
	for _, m := range res {
		next := m[1]
		bags[key] = append(bags[key], next)
	}
}

func hasColor(nexts []string, color string) bool {
	for _, n := range nexts {
		if n == color {
			return true
		}
...
func countBagOfColor(bags map[string]NextBags, color string) int {
	bagsMatch := 0

	// for , inside := range bags {
	// 	for _, col := range inside {
	// 		countBagOfColorRecur(bags, color, col, &bagsMatch)
	// 	}
	// }

	fmt.Println("len = ", len(bags))

	for col := range bags {
		ok := false
		fmt.Println("call for ", col, bagsMatch)
		countBagOfColorRecur(bags, color, col, &ok)
		if ok {
			bagsMatch++
		}
	}
...
func countBagOfColorRecur(bags map[string]NextBags, searchColor, examineColor string, ok *bool) {
	if bags[examineColor] == nil {
		return
	}
	nexts := bags[examineColor]
	if hasColor(nexts, searchColor) {
		if *ok == false {
			*ok = true
		}
	} else {
		for _, n := range nexts {
			countBagOfColorRecur(bags, searchColor, n, ok)
		}
	}
}
...
func main() {
	...
	fmt.Println(countBagOfColor(bags, "shiny gold"))
}
