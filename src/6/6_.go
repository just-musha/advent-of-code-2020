...
func countYes(gr string) int {
	answers := strings.Split(gr, "\n")

	yes := make(map[rune]int, 0)

	for i := 0; i < len(answers); i++ {
		for j := 0; j < len(answers[i]); j++ {
			key := rune(answers[i][j])
			if _, ok := yes[key]; ok {
			}
			yes[key] += 1
		}
...
func main() {
	gr := strings.Split(data, "\n\n")

	totalYesCnt := 0

	for _, g := range gr {
		totalYesCnt += countYes(g)
	}
	fmt.Println("res = ", totalYesCnt)
}
