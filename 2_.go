...
func passIsValid(pas string, sym uint8, l, h int) bool {
	if h > len(pas) || l > len(pas) || h < 1 || l < 1 {
		return false
	}
	...
	if (pas[ih] == sym || pas[il] == sym) && (pas[ih] != pas[il]) {
		return true
	}
	return false
}
...
func main() {
	...
	res := strings.Split(passwd, "\n")
	re := regexp.MustCompile("(\\d+)-(\\d+)\\s([a-z]):\\s([a-z]+)")

	for _, str := range res {
		match := re.FindStringSubmatch(str)
		if len(match) == 0 {
			fmt.Printf("error: cannot extract values from %s\n", str)
			os.Exit(1)
		}
		fmt.Println("Match: ")
		for _, m := range match {
		...
		if err != nil {
			fmt.Printf("error: cannot convert %s to int\n", match[1])
			os.Exit(1)
		}
		...
		if passIsValid(pas, sym, l, h) {
			validPassCnt++
		}
	}

	fmt.Printf("valid passwords count: %d (total: %d)\n", validPassCnt, len(res))
}

var passwd = `2-4 p: vpkpp
