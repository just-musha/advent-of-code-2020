...
type NextBags map[string]int

func parseBagsInfo(bags map[string]NextBags, str string) {
	pattern := " bags contain "

	s := strings.Index(str, pattern)
	key := str[:s]

	cur := str[s+len(pattern):]
	if cur == "no other bags." {
		bags[key] = nil
		return
	}
	re := regexp.MustCompile("(\\d) (.*?) bag")
	res := re.FindAllStringSubmatch(cur, -1)
	...
	for _, m := range res {
		...
		if err != nil {
			fmt.Printf("error: connot convert to int %s\n", cntStr)
			os.Exit(1)
		}
		bags[key][next] = cnt
	}
...
func countAllBags(bags map[string]NextBags, color string) int {
	if bags != nil {
		//fmt.Println("LEN = ", len(bags[color]))
		return countBagsRecur(bags, color) - 1
	}
	return -1
}

func countBagsRecur(bags map[string]NextBags, color string) int {
	if bags[color] == nil {
		return 1
	}

	oneBagMap := bags[color]

	result := 0
	for col, cnt := range oneBagMap {
		result += cnt * countBagsRecur(bags, col)
		//fmt.Printf("col: %s, cnt : %d, res: %d\n", col, cnt, result)
	}
	return result + len(oneBagMap) + 1 - len(bags[color])
}
...
func main() {
	...
	// fmt.Println("Map:")
	// for b, val := range bags {
	// 	fmt.Println(b, ":", val)
	// }

	fmt.Println(countAllBags(bags, "shiny gold"))
}

var test4 = `shiny gold bags contain 3 dark red bags, 5 dark green bags.
dark red bags contain 4 dark violet bags, 3 dark blue bags.
dark green bags contain 2 dark blue bags.
dark blue bags contain 2 dark violet bags.
dark violet bags contain no other bags.`

var test3 = `shiny gold bags contain 3 dark red bags, 5 dark green bags.
dark red bags contain 4 dark violet bags.
dark green bags contain 2 dark blue bags.
dark blue bags contain 2 dark violet bags.
dark violet bags contain no other bags.`

var test2 = `shiny gold bags contain 3 dark red bags, 5 dark green bags.
dark red bags contain 4 dark violet bags.
dark green bags contain 2 dark violet bags.
dark violet bags contain no other bags.`

var test1 = `shiny gold bags contain 7 dark red bags.
dark red bags contain 5 dark violet bags.
dark violet bags contain no other bags.`

var test = `shiny gold bags contain 2 dark red bags.
...
var data = `striped orange bags contain 1 vibrant green bag, 5 plaid yellow bags, 1 drab magenta bag.
