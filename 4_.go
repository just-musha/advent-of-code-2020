...
func is4digitMinMax(pass string, regex string, min, max int) bool {
	re := regexp.MustCompile(regex)
	byrStr := re.FindStringSubmatch(pass)

	if len(byrStr) == 0 {
		return false
	}

	byr, err := strconv.Atoi(byrStr[0][4:8])
	if err != nil {
		fmt.Println("error: cannot convert to Int: ", err)
		os.Exit(1)
	}
...
func isValid(pass string) bool {
	pass += " "
	if !is4digitMinMax(pass, "byr:(\\d{4})[\n ]", 1920, 2002) ||
		!is4digitMinMax(pass, "iyr:(\\d{4})[\n ]", 2010, 2020) ||
		!is4digitMinMax(pass, "eyr:(\\d{4})[\n ]", 2020, 2030) {
		return false
	}

	ok, err := regexp.MatchString("ecl:(amb|blu|brn|gry|grn|hzl|oth)[\n ]", pass)
	if err != nil {
	...
	ok, err = regexp.MatchString("pid:\\d{9}[\n ]", pass)
	if err != nil {
		fmt.Println("error:", err)
		os.Exit(1)
	}
	...
	ok, err = regexp.MatchString("hcl:#[0-9a-f]{6}[\n ]", pass)
	if err != nil {
		fmt.Println("error:", err)
		os.Exit(1)
	}
	...
	re := regexp.MustCompile("hgt:(\\d+)(in|cm)[\n ]")
	hgtStr := re.FindStringSubmatch(pass)
...
func main() {
	...
	fmt.Println("res ", validPassCnt)
	fmt.Println("all ", len(ps))
}

var passports = `eyr:2029 pid:157374862
